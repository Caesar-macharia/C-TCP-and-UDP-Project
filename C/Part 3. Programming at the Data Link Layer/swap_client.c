...
int swap_write(int sd, unsigned char *buf, int length)
{
	...
	frame[0] = s+'0';				//frame sequence number is added as the first element of frame array
	...
	/*compute received ack's chesksum, if wrong, drop it and send last frame*/
			...
			checkAck=checksum(ack,4);		//cheecksum only computes first 4 char
	...
	//if checksum matches sender's checksum and ACK has right next frame#--> send another frame by calling this function again
			...
			{
				printf("\nACK and checksum received correctly from the server. To send another frame\n");
				return nRcv;		//return the length sent and stop loop
			}

			if(checkAck==ack[4] && ack[0]==(s-1+'0'))	//if receive duplicate ACK
			{
				printf("Receive ACK expect this frame again. Send this frame again.");
				s--;			//retun 0; terminate this function, start new write again
			}
