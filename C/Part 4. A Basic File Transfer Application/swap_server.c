...
int swap_read(int sd, char *buf)
{
	...
	n=sdp_receive(sd,frame);		//server receive frame from client

	frameLen=n;

//	printf("swap_read from client:%s. Frame length received is %d ",frame,frameLen );

	/*strip the frame to get sequence#(head) and checksum (tail)*/
	//expected sequence :r+'0': convert int r to unsigned char because frame conatains char

		//received frame length -2:-1 checksum char, -1'\0': only computer lenght==11
	checkRcv=checksum(frame,11);		//compute checksum received from frame except sent checksum
	...
	if( (n>0) && (frame[0]==r+'0') && (frame[11]==checkRcv) )
	{
		...
		/*populate passed i parameter: but with data in received frame*/
		...
		ack[4]=cheSumOut; 		//add sender's checksum ack tail


		sdp_send(sd,ack,5);		//send out ACK to client with expected next frame seqence#
						//ack total 5 char, head is next frame# expected to receive, tail is checksum
		return n;
	}

	//if receive f# is not expected f#: server send last ack again, do not renew buf
	if((n>0) && (frame[0]!=r+'0') && (frame[11]==checkRcv) )
	{
	...
	else
	{
		...
		//else: return n: usually returns negative n to stop fta_server to swap_read() again
